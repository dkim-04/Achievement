...
def stepplot_for_all_columns(file_path, x_column, output_folder):
    ...
    for y_column in y_columns:
        ...
        plt.step(df_y_cleaned[x_column], df_y_cleaned[y_column], where='mid', label=y_column)
        plt.title(y_column, fontsize=16, fontweight='bold')
        plt.xlabel('Time')
        plt.ylabel(y_column)
        plt.grid(True)
        plt.legend()
...
output_folder = '/mnt/c/Users/kim/P_Ver1/data_semicoductor/raw_data/시각 자료/계단형/'

# 데이터 생성 및 저장
stepplot_for_all_columns(file_path, 'TIME', output_folder)